#!/system/bin/sh
# crash_guard.sh — brick-prevention watchdog for SMB400 Hotarun.
#
# Background:
...
#   1. If crash_dump32 count > CD_MAX  -> kill the chain + Hotarun.
...
#   3. If MemAvailable < MEM_FLOOR_KB  -> kill Hotarun as last resort before OOM.
...
MEM_FLOOR_KB=350000    # kill Hotarun if MemAvailable still below this after reclaim
...
# Record our PID for the singleton check in start_hotarun.sh.
...
while true; do
    ...
    if [ "$cd_count" -gt "$CD_MAX" ]; then
        echo "[crash_guard] crash_dump32 count=$cd_count > $CD_MAX — killing chain + Hotarun" >> "$LOG"
        pkill -9 crash_dump32 2>/dev/null
        pkill -9 -f "hotarun" 2>/dev/null
    fi
    ...
    # --- 3. hard floor: kill Hotarun if still critical after reclaim ---
    if [ "$mem" -lt "$MEM_FLOOR_KB" ]; then
        echo "[crash_guard] MemAvailable=${mem}kB < ${MEM_FLOOR_KB}kB — killing Hotarun to prevent OOM" >> "$LOG"
        pkill -9 -f "hotarun" 2>/dev/null
        pkill -9 crash_dump32 2>/dev/null
    fi
